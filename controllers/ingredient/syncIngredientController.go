...
func SyncDataWithAPI(code_entreprise string, pos_id string) {
    ...
    if  len(externalDataList) > 0 {
        for _, externalData := range externalDataList {
            var localData models.Ingredient
    ...
    // Fetch local data
    var localDataList []models.Ingredient
...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(code_entreprise string, pos_id string) ([]models.Ingredient, error) {
    // Replace with the actual URL of your API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/ingredients/%s/%s/all", code_entreprise, pos_id)
...
func fetchExternalDataItemFromAPI(dataID uint) (models.Ingredient, error) {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/ingredients/get/%d", dataID)
    ...
    if resp.StatusCode != http.StatusOK {
        return models.Ingredient{}, fmt.Errorf("failed to fetch data: %s", resp.Status)
    } 
...
func sendLocalDataToAPI(data models.Ingredient) error {
    // Soumission des données vers l'API
    apiURL := "https://i-pos-restaurant-api.up.railway.app/api/ingredients/create"
...
func updateExternalDataInAPI(data models.Ingredient) error {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/ingredients/update/%d", data.ID)
...
func deleteExternalDataInAPI(dataID uint) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/ingredients/delete/%d", dataID)
...
func isEqual(a, b models.Ingredient) bool {
	return a.ID == b.ID &&
		a.Name == b.Name &&
