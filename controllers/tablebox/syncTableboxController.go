...
func SyncDataWithAPI(code_entreprise string, pos_id string) {
    ...
    // Fetch local data
    var localDataList []models.TableBox
...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(code_entreprise string, pos_id string) ([]models.TableBox, error) {
    // Replace with the actual URL of your API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/table-box/%s/%s/all", code_entreprise, pos_id)
    ...
    var response struct {
		Data []models.TableBox `json:"data"`
	}
...
func fetchExternalDataItemFromAPI(dataID uint) (models.TableBox, error) {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/table-box/get/%d", dataID)

    resp, err := http.Get(apiURL)
    if err != nil {
        return models.TableBox{}, err
    }
    defer resp.Body.Close()

    if resp.StatusCode != http.StatusOK {
        return models.TableBox{}, fmt.Errorf("failed to fetch data: %s", resp.Status)
    }
...
func sendLocalDataToAPI(data models.TableBox) error {
    // Soumission des données vers l'API
    apiURL := "https://i-pos-restaurant-api.up.railway.app/api/table-box/create"
...
func updateExternalDataInAPI(data models.TableBox) error {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/table-box/update/%d", data.ID)
...
func deleteExternalDataInAPI(dataID uint) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/table-box/delete/%d", dataID)
...
func isEqual(a, b models.TableBox) bool {
	return a.ID == b.ID &&
		a.Name == b.Name &&
