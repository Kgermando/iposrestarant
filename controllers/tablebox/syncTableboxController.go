...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(code_entreprise string, pos_uuid string) ([]models.TableBox, error) {
	// Replace with the actual URL of your API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/table-box/%s/%s/all", code_entreprise, pos_uuid)
...
func fetchExternalDataItemFromAPI(dataUUID string) (models.TableBox, error) {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/table-box/get/%s", dataUUID)
...
func updateExternalDataInAPI(data models.TableBox) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/table-box/update/%s", data.UUID)
