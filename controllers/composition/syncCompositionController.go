...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(code_entreprise string, pos_uuid string) ([]models.Composition, error) {
	// Replace with the actual URL of your API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/compositions/%s/%s/all", code_entreprise, pos_uuid)
	...
	var response struct {
		Data []models.Composition `json:"data"`
	}
...
func fetchExternalDataItemFromAPI(dataUUID string) (models.Composition, error) {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/compositions/get/%s", dataUUID)
...
func updateExternalDataInAPI(data models.Composition) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/compositions/update/%s", data.UUID)
...
func isEqual(a, b models.Composition) bool {
