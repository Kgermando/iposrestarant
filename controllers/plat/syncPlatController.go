...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(code_entreprise string, pos_id string) ([]models.Plat, error) {
    // Replace with the actual URL of your API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/plats/%s/%s/all", code_entreprise, pos_id)
...
func fetchExternalDataItemFromAPI(dataID uint) (models.Plat, error) {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/plats/get/%d", dataID)
...
func updateExternalDataInAPI(data models.Plat) error {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/plats/update/%d", data.ID)
