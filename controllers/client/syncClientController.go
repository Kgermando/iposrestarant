...
func fetchExternalDataItemFromAPI(dataUUID string) (models.Client, error) {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/clients/get/%s", dataUUID)
...
func updateExternalDataInAPI(data models.Client) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/clients/update/%s", data.UUID)
