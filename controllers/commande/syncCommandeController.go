...
func SyncDataWithAPI(code_entreprise string, pos_id string) {
    ...
    // Fetch local data
    var localDataList []models.Commande
...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(code_entreprise string, pos_id string) ([]models.Commande, error) {
    // Replace with the actual URL of your API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/commandes/%s/%s/all", code_entreprise, pos_id)
...
func fetchExternalDataItemFromAPI(dataID uint) (models.Commande, error) {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/commandes/get/%d", dataID)

    resp, err := http.Get(apiURL)
    if err != nil {
        return models.Commande{}, err
    }
    defer resp.Body.Close()

    if resp.StatusCode != http.StatusOK {
        return models.Commande{}, fmt.Errorf("failed to fetch data: %s", resp.Status)
    }
...
func sendLocalDataToAPI(data models.Commande) error {
    ...
    apiURL := "https://i-pos-restaurant-api.up.railway.app/api/commandes/create"
...
func updateExternalDataInAPI(data models.Commande) error {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/commandes/update/%d", data.ID)
...
func deleteExternalDataInAPI(dataID uint) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/commandes/delete/%d", dataID)
...
func isEqual(a, b models.Commande) bool {
	return a.ID == b.ID &&
		a.Ncommande == b.Ncommande &&
