...
func fetchExternalDataItemFromAPI(dataUUID string) (models.Commande, error) {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/commandes/get/%s", dataUUID)
...
func updateExternalDataInAPI(data models.Commande) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/commandes/update/%s", data.UUID)
