...
func SyncDataWithAPI(product_id string) {
    mu.Lock()
    defer mu.Unlock()
    ...
    externalDataList, err := fetchExternalDataFromAPI(product_id)
    if err != nil {
...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(product_id string) ([]models.Stock, error) {
    // Replace with the actual URL of your API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/stocks/all/%s", product_id)
...
func fetchExternalDataItemFromAPI(dataID uint) (models.Stock, error) {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/stocks/get/%d", dataID)
    ...
    if resp.StatusCode != http.StatusOK {
        return models.Stock{}, fmt.Errorf("failed to fetch data: %s", resp.Status)
    } 
...
func sendLocalDataToAPI(data models.Stock) error {
    ...
    apiURL := "https://i-pos-restaurant-api.up.railway.app/api/stocks/create"
...
func updateExternalDataInAPI(data models.Stock) error {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/stocks/update/%d", data.ID)
...
func deleteExternalDataInAPI(dataID uint) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/stocks/delete/%d", dataID)
