...
func SyncDataWithAPI(commande_id string) {
    mu.Lock()
    defer mu.Unlock()
    ...
    externalDataList, err := fetchExternalDataFromAPI(commande_id)
    if err != nil {
...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(commande_id string) ([]models.CommandeLine, error) {
    // Replace with the actual URL of your API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/commandes-lines/all/%s", commande_id)
...
func fetchExternalDataItemFromAPI(dataID uint) (models.CommandeLine, error) {
    // URL de l'API
...
func sendLocalDataToAPI(data models.CommandeLine) error {
...
func updateExternalDataInAPI(data models.CommandeLine) error {
    // URL de l'API
...
func deleteExternalDataInAPI(dataID uint) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/commandes-lines/delete/%d", dataID)
...
func isEqual(a, b models.CommandeLine) bool {
	return a.ID == b.ID &&
		a.CommandeID == b.CommandeID &&
