...
func SyncDataWithAPI(commande_uuid string) {
	mu.Lock()
	defer mu.Unlock()

	// Récupérer des données externes à partir de l'API
	externalDataList, err := fetchExternalDataFromAPI(commande_uuid)
	if err != nil {
...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(commande_uuid string) ([]models.CommandeLine, error) {
	// Replace with the actual URL of your API
	// apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/commandes-lines/all/%s", commande_uuid)
	...
	db.Where("commande_lines.commande_uuid = ?", commande_uuid).Find(&dataList)

	return dataList, nil
}

// Récupérer une donnee externe à partir de l'API
func fetchExternalDataItemFromAPI(dataUUID string) (models.CommandeLine, error) {
	db := database.PGDB
