...
func SyncDataWithAPI(code_entreprise string, pos_uuid string) {
	...
	if len(externalDataList) > 0 {
		for _, externalData := range externalDataList {
			var localData models.Livraison
	...
	// Fetch local data
	var localDataList []models.Livraison
...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(code_entreprise string, pos_uuid string) ([]models.Livraison, error) {
	// Replace with the actual URL of your API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/livraisons/%s/%s/all", code_entreprise, pos_uuid)
...
func fetchExternalDataItemFromAPI(dataUUID string) (models.Livraison, error) {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/livraisons/get/%s", dataUUID)
	...
	if resp.StatusCode != http.StatusOK {
		return models.Livraison{}, fmt.Errorf("failed to fetch data: %s", resp.Status)
	}
...
func sendLocalDataToAPI(data models.Livraison) error {
	// Soumission des données vers l'API
	apiURL := "https://i-pos-restaurant-api.up.railway.app/api/livraisons/create"
...
func updateExternalDataInAPI(data models.Livraison) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/livraisons/update/%s", data.UUID)
...
func isEqual(a, b models.Livraison) bool {
