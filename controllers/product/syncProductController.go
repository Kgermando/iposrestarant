...
func SyncDataWithAPI(code_entreprise string, pos_id string) {
    ...
    // Fetch local data
    var localDataList []models.Product
...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(code_entreprise string, pos_id string) ([]models.Product, error) {
    // Replace with the actual URL of your API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/products/%s/%s/all", code_entreprise, pos_id)
...
func fetchExternalDataItemFromAPI(dataID uint) (models.Product, error) {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/products/get/%d", dataID)
...
func updateExternalDataInAPI(data models.Product) error {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/products/update/%d", data.ID)
...
func isEqual(a, b models.Product) bool {
	return a.ID == b.ID &&
		a.Name == b.Name &&
