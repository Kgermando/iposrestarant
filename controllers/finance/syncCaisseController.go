...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(code_entreprise string, pos_id string) ([]models.Caisse, error) {
    // Replace with the actual URL of your API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/caisses/%s/%s/all", code_entreprise, pos_id)
...
func fetchExternalDataItemFromAPI(dataID uint) (models.Caisse, error) {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/caisses/get/%d", dataID)
...
func updateExternalDataInAPI(data models.Caisse) error {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/caisses/update/%d", data.ID)
