...
func fetchExternalDataItemFromAPI(dataUUID string) (models.Livreur, error) {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/livreurs/get/%s", dataUUID)
...
func updateExternalDataInAPI(data models.Livreur) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/livreurs/update/%s", data.UUID)
...
func isEqual(a, b models.Livreur) bool {
