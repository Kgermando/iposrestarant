...
func SyncDataWithAPI(code_entreprise string) {
    ...
    if  len(externalDataList) > 0 {
        for _, externalData := range externalDataList {
            var localData models.Fournisseur
    ...
    // Fetch local data
    var localDataList []models.Fournisseur
...
// Récupérer des données externes à partir de l'API
func fetchExternalDataFromAPI(code_entreprise string) ([]models.Fournisseur, error) {
    // Replace with the actual URL of your API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/fournisseurs/%s/all", code_entreprise)
...
func fetchExternalDataItemFromAPI(dataID uint) (models.Fournisseur, error) {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/fournisseurs/get/%d", dataID)
    ...
    if resp.StatusCode != http.StatusOK {
        return models.Fournisseur{}, fmt.Errorf("failed to fetch data: %s", resp.Status)
    }
...
func sendLocalDataToAPI(data models.Fournisseur) error {
    // Soumission des données vers l'API
    apiURL := "https://i-pos-restaurant-api.up.railway.app/api/fournisseurs/create"
...
func updateExternalDataInAPI(data models.Fournisseur) error {
    // URL de l'API
    apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/fournisseurs/update/%d", data.ID)
...
func deleteExternalDataInAPI(dataID uint) error {
	// URL de l'API
	apiURL := fmt.Sprintf("https://i-pos-restaurant-api.up.railway.app/api/fournisseurs/delete/%d", dataID)
...
func isEqual(a, b models.Fournisseur) bool {
	return a.ID == b.ID &&
		a.Name == b.Name &&
